...
def fill_and_detect_gamePiece(mask: cv2.UMat, min_area=MIN_AREA):
    ...
    for contour in contours:
        ...
        if area > min_area and len(contour) >= 5:
            ...
            x, y, w, h = cv2.boundingRect(contour)
            box = get_rectangle_points(contour)

            if len(box) > 3:
                angle = get_rotation_angle(box[0], box[3])
...
def display_rotated(frame, box):
    for i, b in enumerate(box):
        cv2.putText(frame, f'{i}, {b}', b, 5, 1, [0, 255, 0])
        
    cv2.drawContours(frame, [box.astype(int)], 0, (0, 0, 255), 2)

def display_angle(frame, angle):
    cv2.putText(frame, f'Angle: {angle}', (50, 100), 1, 1, [0, 0, 255], 4)

def get_rotation_angle(b0, b3):
    x1, y1 = b0 
    x2, y2 = b3
    m = (y1 - y2) / (x1 - x2)
    angle = np.atan(m)
